...
Last updated: 2026-03-28 (session 31)
...
|---------|------|------------|
| v1.16.1 | 2026-03-28 | Fix daemon crash-loop on invalid refresh token: add `"invalid refresh token"` to permanent auth errors (exits cleanly instead of looping), atomic config writes via temp+rename to prevent token loss on crash/kill. |
...
|---------|------|---------|
| 31 | 2026-03-28 | Fix daemon crash-loop from invalid refresh token (restart counter hit 8134 on fedora-1). Two bugs: (1) `"invalid refresh token"` not in `permanentAuthErrors` → daemon exited with code 1 → systemd restarted forever; fixed by adding it to the permanent error list so daemon exits cleanly (code 0). (2) Token loss window between server rotation and disk write → `writeFile()` now uses atomic temp+rename instead of `os.WriteFile()`. |
