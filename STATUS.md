...
- **2026-02-12 (session 9)**: Add MIT license, Homebrew tap via GoReleaser, and GitHub App-based token for cross-repo formula publishing. Project transferred from `kuuji/bamgate` to `bamgate` GitHub org. GoReleaser `brews` section auto-publishes formula to `bamgate/homebrew-tap` on release. Release workflow uses `actions/create-github-app-token` to generate ephemeral tokens (no PAT expiry). Users can now install via `brew install bamgate/tap/bamgate`.
